...
function initialParenTrail() {
  return {
    ...
    leading: false,          // [boolean] - is this a leading paren trail? (i.e. no tokens before it on the line)
...
function deleteLine(result) {
  result.lineNo--;
  result.lines.pop();
}

function insertLine(result, lineNo, line) {
  // TODO: insert line in result.lines
  // TODO: update all affected line numbers (e.g. cursor, openers, parens)
}
...
function resetParenTrail(result, lineNo, x) {
  ...
  result.parenTrail.leading = false;
...
function flushParenTrail(result, parens) {
  if (result.parenTrail.leading) {
    // TODO: insert new line if needed (i.e. if indentedComment)
    // TODO: ensure line has indentation spaces (so replaceWithinLine works)
    // TODO: replaceWithinLine
    // TODO: rememberParenTrail
  }
  else if (result.parenTrail.lineNo !== UINT_NULL) {
...
function correctParenTrail(result, indentX) {
  ...
  var parentIndex = getParentOpenerIndex(result, indentX);
  var i;
  for (i=0; i<parentIndex; i++) {
    ...
    if (opener.indentedComment) {
      flushParenTrail(result, parens);
      parens = "";
      result.parenTrail.startX = opener.indentedComment.x;
      result.parenTrail.endX = opener.indentedComment.x;
      result.parenTrail.lineNo = opener.indentedComment.lineNo+1;
      result.parenTrail.leading = true;
    }

    if (result.returnParens) {
      setCloser(opener, result.parenTrail.lineNo, result.parenTrail.startX+parens.length, closeCh);
    }
    if (result.parenTrail.leading) {
  ...
  // remove indentedComment from each opener not containing this indentation point
  for (i=0; i<result.parenStack.length; i++) {
    var opener = peek(result.parenStack, i);
    delete opener.indentedComment;
  }

  flushParenTrail(result, parens, leadingTrail);
}
...
function onLeadingCloseParen(result) {
  var parenTrailLine = result.inputLines[result.inputLineNo].match(/^([\s\]\)\}]*)(;.*)?$/);
  var parenTrailComment = parenTrailLine && parenTrailLine[2];

  if (result.mode === INDENT_MODE) {
    result.skipChar = true;

    if (parenTrailLine) {
      // FIXME: removing and reinserting leading paren trails will result in
      ...
      //    ```
      //
      // TODO: point indentedComment to the first line containing a leading paren trail
      //       (or line containing cursor in case whole paren trail was deleted by it)
      //       (a cursor line should only be used if no leading paren trail line is found yet,
      //        but should be overwritten by the next paren trail line)

      if (parenTrailComment) {
      }
      else {
      }

      // OLD VERSION that just deletes the parenTrail line that causes the aforementioned FIXME problems.
      // if (parenTrailLine && !parenTrailComment && result.cursorLine !== result.lineNo) {
      //   deleteLine(result);
      //   throw {skipLine: true};
      // }
    }
    ...
    else if (!result.errorPosCache[ERROR_LEADING_CLOSE_PAREN]) {
    ...
    if (!isValidCloseParen(result.parenStack, result.ch)) {
      ...
      // if (opener.indentedComment) {
      //   addIndent(indentedComment.x-result.x);
      //   resetParenTrail
      //   onIndent  (or some variant since addIndent might be mostly sufficient)
      // else {
      //   appendParenTrail
      //   if parenTrailLine[1].trim().length === 1 && !parenTrailComment, deleteLine
      // }
